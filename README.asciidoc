...
`stdout/stderr` in a single log file, be sure to run *image-bootstrap* in
unbuffered mode, e.g.:

----------------------------------------------------
python -u image-bootstrap [params] 2>&1 | tee my.log
----------------------------------------------------
...
https://github.com/hartwork/image-bootstrap/issues/71 for more details)
...
invoking *image-bootstrap*. (Please google `bash pipefail` for more details.)
